...
#ifndef MODIFY_ZOU_HE_DYNAMICS_HH
#define MODIFY_ZOU_HE_DYNAMICS_HH
...
void modifyZouHeClosure ( Cell<T,Descriptor>& cell, Dynamics<T,Descriptor> const& dynamics )
{
    ...
    //  f[5] = feq[5] + fneq[1])
    cell.getExternal(Descriptor<T>::ExternalField::densityBeginsAt)
    for (pluint iPop = 0; iPop < missingIndexes.size(); ++iPop)
    {
        plint pop = missingIndexes[iPop];
        plint opp = indexTemplates::opposite<L>(pop);
        cell[pop] = cell[opp]
            - dynamics.computeEquilibrium(opp, rhoBar, j, jSqr, A)
            + dynamics.computeEquilibrium(pop, rhoBar, j, jSqr, A);
    }
